...
function SubmissaoApp() {
  ...
  const [ajusteLayout, setAjusteLayout] = useState("");
  ...
  useEffect(()=>{
    ...
    try {
      ...
      if (typeof d.ajusteLayout === "string") setAjusteLayout(d.ajusteLayout);
  ...
  useEffect(()=>{
    ...
    const id = setTimeout(()=>{
      try {
        localStorage.setItem(RASCUNHO_KEY, JSON.stringify({ f, figuras, principal, fotoAutores, ajusteLayout }));
      } catch (e) {
        try { localStorage.setItem(RASCUNHO_KEY, JSON.stringify({ f, figuras: figuras.map(({ dataUrl, ...r })=>r), principal, semImagens: true, ajusteLayout })); } catch (e2) {}
      }
    }, 800);
  ...
  const previewT = { ...f, fase:Number(f.fase), foto_autores_dataUrl: fotoAutores, figPrincipalUrl: figPrincipal && figPrincipal.dataUrl, figLegenda: figPrincipal && figPrincipal.legenda, figTitulo: figPrincipal && figPrincipal.titulo, figuras: figuras.map((fg,i)=>({ ordem:i+1, secao:fg.secao, legenda:fg.legenda, titulo:fg.titulo, dataUrl:fg.dataUrl, principal:i===principal })), fig_principal: principal+1, ajuste_layout: ajusteLayout };
  const notas = {
      ...
      {aba==="form" && <div style={{ maxWidth:1280, margin:"0 auto", padding:"20px", display:"grid", gridTemplateColumns:"minmax(320px, 1.4fr) minmax(250px, 0.9fr)", gap:22, alignItems:"start" }} className="sub-grid">
        ...
        <div style={{ background:"#fff", border:"1px solid #E3EAF2", borderRadius:14, padding:20 }}>
          ...
          {!ehFase8 && <>
            ...
            {/* Ver na TV — overlay fullscreen com edição ao vivo das figuras (itens 3+4) */}
            <button type="button" onClick={()=>window.SAM_PREVIA && window.SAM_PREVIA.abrir({ trabalho: previewT, editavel: true, salvarLabel: "Aplicar ao rascunho", onSalvar: (str)=>{ setAjusteLayout(str); return "Ajuste aplicado ao rascunho."; } })} style={{ width:"100%", marginTop:4, background:"#fff", color:C.azul, border:`1px solid ${C.azul}`, borderRadius:10, padding:"11px", fontSize:14, fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
              <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke={C.azul} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/></svg>
