...
const PosterEditor = window.PosterEditor; /* editor de layout TV — carregado por poster-editor.jsx */
...
function SubmissaoApp() {
  ...
  const enviar = async () => {
    ...
    const payload = {
      ...
      ajuste_layout:Object.keys(ajustesLayout).length ? JSON.stringify(ajustesLayout) : undefined,
    };
        ...
        <div className="col-preview" style={{ position:"sticky", top:16 }}>
          {!ehFase8 ? <>
            <div style={{ display:"flex", gap:6, marginBottom:10, flexWrap:"wrap" }}>
              ...
              {PosterEditor && <button onClick={()=>setVistaTV(true)} style={{ border:'none', borderRadius:8, padding:'6px 10px', fontSize:11.5, fontWeight:700, cursor:'pointer', background:C.azul, color:'#fff', display:'inline-flex', alignItems:'center', gap:5 }}><Monitor size={13} color='#fff'/> TV (editor)</button>}
            </div>
      ...
      {vistaTV && !ehFase8 && PosterEditor && (
        <div style={{ position:'fixed', inset:0, zIndex:50, background:'#08131E', display:'flex', flexDirection:'column' }}>
          <div style={{ flexShrink:0, background:C.azulEsc, color:'#fff', padding:'0 16px', height:50, display:'flex', alignItems:'center', gap:12, position:'relative', zIndex:5 }}>
            <button onClick={()=>setVistaTV(false)} style={{ background:'transparent', border:'none', color:'#fff', cursor:'pointer', display:'inline-flex', alignItems:'center', gap:6, fontSize:13, fontWeight:600, fontFamily:'inherit' }}>← Voltar</button>
            <span style={{ flex:1, fontSize:13, color:'rgba(255,255,255,0.6)' }}>Visualização TV · ajuste o layout das figuras e salve</span>
            {Object.keys(ajustesLayout).length>0 && <span style={{ fontSize:12, color:C.ciano, fontWeight:700 }}>● Ajustado</span>}
          </div>
          <div style={{ flex:1, overflow:'hidden', position:'relative' }}>
            <PosterEditor
              t={{ ...f, fase:Number(f.fase), foto_autores_dataUrl:fotoAutores,
                   figuras:figuras.map((fg,i)=>({...fg,url:fg.dataUrl,ordem:i+1,principal:i===principal})) }}
              initialAjustes={ajustesLayout}
              onSave={(aj)=>{ setAjustesLayout(aj); setVistaTV(false); }}
              onCancel={()=>setVistaTV(false)}
            />
          </div>
        </div>
      )}
